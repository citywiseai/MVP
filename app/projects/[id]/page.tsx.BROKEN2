...
async function createTask(formData: FormData) {
  ...
  await prisma.task.create({
    data: {
      ...
      title,
      description,
  // createdBy: { connect: { id: user.id } }, // commented out, not in schema
      status: 'TODO',
    }
  })
...
async function createNote(formData: FormData) {
  ...
  await prisma.note.create({
    data: {
      projectId,
      content,
  // author: { connect: { id: user.id } }, // commented out, not in schema
    }
  })
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {(project as any).parcel && (
          <PropertyVisualization
            // projectId={project.id} // Uncomment if needed and supported
            parcelData={{
              // latitude: (project as any).parcel.latitude, // commented out, not in schema
              // longitude: (project as any).parcel.longitude, // commented out, not in schema
              boundaryCoordinates: [], // minimal stub
              zoningRules: [] // minimal stub
            }}
            // initialSetbacks={{
            ...
            // }}
            // platMapUrl={undefined}
            // subdivisionName={undefined}
            // platReference={undefined}
          />
        )}
